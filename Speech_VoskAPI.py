...
def callback(indata, frames, time, status):
    global is_speaking, last_voice_time, speech_buffer
    
    if status:
        print("Audio Error:", status)
    
    volume = calculate_volume_level(indata)
...
def detect_best_match(text, category, threshold=80):
    """Find best keyword match using fuzzy matching"""
    ...
    with sd.RawInputStream(samplerate=samplerate, blocksize=8000, dtype='int16',
        ...
        while True:
            try:
                ...
                # No audio data in queue (normal during silence)
                continue
